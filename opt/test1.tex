...
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{scp_sensitivity_trajectories.png-c46a9db6-a2b9-425d-a3ce-4a6c0afdd505} % Reemplazar con la imagen real
    \caption{Trayectorias de la simulación de línea base (Baseline) para SCP y OpEn.}
    \label{fig:trayectoria}
\end{figure}

\subsection{Eficiencia Computacional y Perfiles de Control}
La diferencia en tiempos de cómputo es drástica (ver Fig. \ref{fig:tiempos}). OpEn logra un tiempo medio de ejecución de \textbf{1.11 ms}, lo cual es \textbf{más de 750 veces más rápido} que la implementación de SCP en Python. Esto se debe a que OpEn ejecuta código Rust nativo sin asignación dinámica de memoria. Los perfiles de control (ver Tabla \ref{tab:comparacion}) muestran que ambos métodos alcanzan una solución comparable en términos de pasos de simulación (29 vs 30).

\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{IMG/tiempo_computo.png}
    \caption{Tiempo de cómputo por paso de simulación en escala logarítmica (Placeholder).}
    \label{fig:tiempos}
\end{figure}
...
% ------------------------------------------------------------
\section{Análisis de Sensibilidad y Monte Carlo}

Para evaluar la robustez del algoritmo SCP implementado, se realizó un estudio de sensibilidad Monte Carlo de $N_{trials}=25$ simulaciones para seis escenarios distintos:
\begin{itemize}
    \item \textbf{Baseline:} Parámetros nominales.
    \item \textbf{Perturbación $x_0$ (P1, P2):} Ligeras variaciones en la posición inicial (e.g., $\Delta x, \Delta y \sim \mathcal{N}(0, 0.02)$).
    \item \textbf{Perturbación $\theta_0$:} Variación en el ángulo inicial.
    \item \textbf{Región de Confianza Apretrada (\textit{Tight Trust} $\Delta_{TR}$):} $\Delta_{TR, x}=0.01$, $\Delta_{TR, u}=0.01$.
    \item \textbf{Región de Confianza Floja (\textit{Loose Trust} $\Delta_{TR}$):} $\Delta_{TR, x}=1.0$, $\Delta_{TR, u}=1.0$ (parámetros usados en \textit{Baseline}).
\end{itemize}

El objetivo es cuantificar el rendimiento en términos de **Distancia Final** al objetivo ($z_{goal}$) y **Pasos de Simulación** para converger. La Tabla \ref{tab:monte_carlo} resume las métricas clave, y la Figura \ref{fig:monte_carlo_trayectorias} ilustra las trayectorias resultantes.

\begin{table}[H]
\caption{Resumen de Resultados Monte Carlo (25 simulaciones)}
\begin{center}
\begin{tabular}{lcccc}
\toprule
\textbf{Escenario} & \textbf{Distancia Final} & \textbf{Std Dist.} & \textbf{Pasos Prom.} & \textbf{Std Pasos} \\
\midrule
Baseline & 0.0782 & 0.0311 & 28.69 & 0.46 \\
x0\_perturb1 & 0.0986 & 0.0241 & 28.08 & 0.27 \\
x0\_perturb2 & 0.0717 & 0.0231 & 29.12 & 0.43 \\
heading\_perturb & 0.0761 & 0.0313 & 28.84 & 0.61 \\
\textbf{tight\_trust} & \textbf{4.2049} & \textbf{0.0251} & \textbf{0.00} & \textbf{0.00} \\
loose\_trust & 0.0784 & 0.0249 & 28.92 & 0.27 \\
\bottomrule
\end{tabular}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{scp_sensitivity_trajectories.png-c46a9db6-a2b9-425d-a3ce-4a6c0afdd505}
    \caption{Comparación de las trayectorias de los diferentes escenarios de sensibilidad (Monte Carlo).}
    \label{fig:monte_carlo_trayectorias}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{scp_monte_25_finaldist_box.png-c29aebc9-4f73-456e-853d-9ff4380d96f8}
    \caption{Boxplot de la distribución de la distancia final al objetivo.}
    \label{fig:monte_carlo_boxplot}
\end{figure}

\subsection{Discusión de Sensibilidad}
Los resultados de perturbación en el estado inicial ($x_0$, $\theta_0$) confirman la **robustez** del SCP, con las distribuciones de distancia final y pasos de simulación siendo muy similares a la línea base (ver Fig. \ref{fig:monte_carlo_boxplot}). Esto valida que la estrategia de linealización y el mecanismo de región de confianza implementado son efectivos ante errores de inicialización y leves errores de estado.

Sin embargo, el escenario de **Región de Confianza Apretrada ($\Delta_{TR}=0.01$ en $z$ y $u$)** tuvo un resultado crítico. Como se observa en la Tabla \ref{tab:monte_carlo}, la distancia final promedio es de $4.20$ m, y el número de pasos es $0.00$, lo que indica que **el planificador no pudo encontrar una trayectoria factible ni ejecutar un solo paso de control**. Una región de confianza demasiado restrictiva impide que el solucionador dé pasos suficientemente grandes para escapar de la solución inicial y converger a una trayectoria óptima. Este resultado subraya la importancia de un tamaño adecuado de $\Delta_{TR}$ para el éxito del SCP en problemas no lineales.
...
\begin{enumerate}
    \item SCP es una estrategia robusta que garantiza el cumplimiento de restricciones duras mediante convexificación sucesiva, demostrando una alta **robustez** ante perturbaciones leves en el estado inicial.
    \item El **tamaño de la Región de Confianza** es crítico, ya que un valor demasiado pequeño ($\Delta_{TR} = 0.01$) causó la inviabilidad del problema.
